...
# batch_size = 32
# Training dataset.
df = pd.read_csv("New_2stage_train.csv", header=None)
x_train = df.values[:, :-1]
y_train = df.values[:, -1].astype(int)


# Validation dataset.
df = pd.read_csv("New_2stage_validate.csv", header=None)
x_validate = df.values[:, :-1]
y_validate = df.values[:, -1].astype(int)

# test dataset.
df = pd.read_csv("New_2stage_test.csv", header=None)
x_test = df.values[:, :-1]
...
estimator = tf1.estimator.DNNClassifier(
   ...
   n_classes=19, # changing the number of classes from 2 to 19 changed this from a sigmoid function to a cross entropy
   dropout=0.1,
   model_dir='2stage_ecg_model'
)
